...
public class AvailableMarketsPrinter {
    ...
    public void print(boolean printMappings) {
        ...
        marketDescriptionManager.getMarketDescriptions()
                .forEach(m -> {
                    logEntry(
                            String.format("Id:'%s', Name:'%s', Associated groups:'%s', Specifiers:'%s'",
                                    m.getId(),
                                    m.getName(Locale.ENGLISH), // by default only the english locale is pre-fetched
                                    m.getGroups(),
                                    m.getSpecifiers() == null ? "" : m.getSpecifiers().stream().map(Specifier::getName).collect(Collectors.joining(","))));

                    if (printMappings && m.getMappings() != null) {
                        logEntry("\tMappingData:");
                        m.getMappings()
                                .forEach(mm -> logEntry(
                                        String.format("\t\tProducerIds:'%s', SportId:'%s', MappedMarketId:'%s', ValidFor:'%s', SovTemplate:'%s'",
                                                mm.getProducerIds(),
                                                mm.getSportId(),
                                                mm.getMarketId(),
